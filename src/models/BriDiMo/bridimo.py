    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.save_images > 0 and batch_idx == 0:
            for i in range(min(self.save_images, predictions.size(0))):
                Path(f"BriDiMo_imgs/originals/{i}").mkdir(parents=True, exist_ok=True)
                Path(f"BriDiMo_imgs/generated/{i}").mkdir(parents=True, exist_ok=True)
                save_image(
                    predictions[i], f"BriDiMo/generated/{i}/{self.current_epoch}.jpg"
                )
                save_image(
                    batch['target'][i], f"BriDiMo/originals/{i}.jpg"
                )  # will be the same through all epochs

        return loss

    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        prediction, _ = self(batch["image"], batch['source_lightness'], batch['target_lightness'])
        return prediction
