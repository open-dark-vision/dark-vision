...
class SmoothLoss(nn.Module):
    ...
    def forward(self, input, output):
        ...
        w1 = torch.exp(
            torch.sum(
                torch.pow(self.input[:, :, 1:, :] - self.input[:, :, :-1, :], 2),
                dim=1,
                keepdim=True,
            )
            * sigma_color
        ...
        w5 = torch.exp(
            torch.sum(
                torch.pow(self.input[:, :, :-1, :-1] - self.input[:, :, 1:, 1:], 2),
                dim=1,
                keepdim=True,
            )
            * sigma_color
        ...
        w9 = torch.exp(
            torch.sum(
                torch.pow(self.input[:, :, 2:, :] - self.input[:, :, :-2, :], 2),
                dim=1,
                keepdim=True,
            )
            * sigma_color
        ...
        w13 = torch.exp(
            torch.sum(
                torch.pow(self.input[:, :, :-2, :-1] - self.input[:, :, 2:, 1:], 2),
                dim=1,
                keepdim=True,
            )
            * sigma_color
        ...
        w17 = torch.exp(
            torch.sum(
                torch.pow(self.input[:, :, :-1, :-2] - self.input[:, :, 1:, 2:], 2),
                dim=1,
                keepdim=True,
            )
            * sigma_color
        ...
        w21 = torch.exp(
            torch.sum(
                torch.pow(self.input[:, :, :-2, :-2] - self.input[:, :, 2:, 2:], 2),
                dim=1,
                keepdim=True,
            )
            * sigma_color
        ...
        pixel_grad1 = w1 * torch.norm(
            (self.output[:, :, 1:, :] - self.output[:, :, :-1, :]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad2 = w2 * torch.norm(
            (self.output[:, :, :-1, :] - self.output[:, :, 1:, :]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad3 = w3 * torch.norm(
            (self.output[:, :, :, 1:] - self.output[:, :, :, :-1]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad4 = w4 * torch.norm(
            (self.output[:, :, :, :-1] - self.output[:, :, :, 1:]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad5 = w5 * torch.norm(
            (self.output[:, :, :-1, :-1] - self.output[:, :, 1:, 1:]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad6 = w6 * torch.norm(
            (self.output[:, :, 1:, 1:] - self.output[:, :, :-1, :-1]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad7 = w7 * torch.norm(
            (self.output[:, :, 1:, :-1] - self.output[:, :, :-1, 1:]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad8 = w8 * torch.norm(
            (self.output[:, :, :-1, 1:] - self.output[:, :, 1:, :-1]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad9 = w9 * torch.norm(
            (self.output[:, :, 2:, :] - self.output[:, :, :-2, :]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad10 = w10 * torch.norm(
            (self.output[:, :, :-2, :] - self.output[:, :, 2:, :]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad11 = w11 * torch.norm(
            (self.output[:, :, :, 2:] - self.output[:, :, :, :-2]),
            p,
            dim=1,
            keepdim=True,
        )
        pixel_grad12 = w12 * torch.norm(
            (self.output[:, :, :, :-2] - self.output[:, :, :, 2:]),
            p,
            dim=1,
            keepdim=True,
        ...
        pixel_grad13 = w13 * torch.norm(
            (self.output[:, :, :-2, :-1] - self.output[:, :, 2:, 1:]),
            p,
            dim=1,
            keepdim=True,
        )
        ...
        pixel_grad17 = w17 * torch.norm(
            (self.output[:, :, :-1, :-2] - self.output[:, :, 1:, 2:]),
            p,
            dim=1,
            keepdim=True,
        )
        ...
        pixel_grad21 = w21 * torch.norm(
            (self.output[:, :, :-2, :-2] - self.output[:, :, 2:, 2:]),
            p,
            dim=1,
            keepdim=True,
        )
...
class SCILoss(nn.Module):
    ...
    def _compute_loss(self, images, illumination):
        fidelity = self.l2_loss(illumination, images)
        smoothness = self.smooth_loss(images, illumination)
    ...
    def _forward_finetune(self, images: torch.Tensor, illumination: torch.Tensor):
